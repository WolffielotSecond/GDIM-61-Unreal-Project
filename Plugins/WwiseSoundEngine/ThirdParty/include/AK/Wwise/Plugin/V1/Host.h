 ...
 * \file AK/Wwise/Plugin/V1/Host.h
 */
...
#endif
{
	...
	ak_wwise_plugin_host_v1(int in_version = 1) :
		ak_wwise_plugin_base_interface(AK_WWISE_PLUGIN_INTERFACE_TYPE_HOST, in_version)
	{}
#endif
...
namespace AK::Wwise::Plugin
{
	namespace V1
	{
		...
		template<typename CHostT = CHost, int interface_version = 1>
		class HostBase : public CBaseInstanceGlue<CHostT>
		{
		public:
			using Interface = CHostT;
			using Instance = typename CHostT::Instance;
			 ...
			 * \return The unique ID of the current platform.
			 */
			inline GUID GetCurrentPlatform() const { return this->g_cinterface->GetCurrentPlatform(this->g_cinterface); }
			 ...
			 * \return The unique ID of the current base platform.
			 */
			inline BasePlatformID GetCurrentBasePlatform() const { return this->g_cinterface->GetCurrentBasePlatform(this->g_cinterface); }
			 ...
			 * \return The unique base ID of the requested playback's platform.
			 */
			inline BasePlatformID GetDefaultNativeAuthoringPlaybackPlatform() const { return this->g_cinterface->GetDefaultNativeAuthoringPlaybackPlatform(this->g_cinterface); }
			 ...
			 * \return The unique ID of the requested playback's platform.
			 */
			inline GUID GetAuthoringPlaybackPlatform() const { return this->g_cinterface->GetAuthoringPlaybackPlatform(this->g_cinterface); }
			 ...
			 * \param[in] in_bMakeProjectDirty Whether project needs saving.
			 */
			inline void NotifyInternalDataChanged(AkPluginParamID in_idData, bool in_bMakeProjectDirty) { this->g_cinterface->NotifyInternalDataChanged(this, in_idData, in_bMakeProjectDirty); }
			...
			{
				this->g_cinterface->GetLicenseStatus(this, &in_guidPlatform, &out_eType, &out_eStatus, &out_uDaysToExpiry);
			}
			...
			{
				this->g_cinterface->GetAssetLicenseStatus(this, &in_guidPlatform, in_uAssetID, &out_eType, &out_eStatus, &out_uDaysToExpiry);
			}
			...
			{
				this->g_cinterface->WaapiCall(this->g_cinterface, in_szUri, in_szArgs, in_szOptions, &in_alloc, &out_szResults, &out_szError);
			}
		};
		...
		namespace Notifications
		{
			...
			public:
				/**
				 * \copydoc ak_wwise_plugin_notifications_host_v1::Instance
				 */
	...
	AK_WWISE_PLUGIN_SPECIALIZE_HOST_INTERFACE(V1::Host, host,, public Notifications::Host);
	AK_WWISE_PLUGIN_SPECIALIZE_INTERFACE_CLASS(V1::Host);
